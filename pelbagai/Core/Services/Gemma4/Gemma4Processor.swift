...
private struct Gemma4PreparedAudio {
    let processed: LMInput.ProcessedAudio
    let sampleCount: Int
    let sampleRate: Int
    let tokenCount: Int
}
...
public struct Gemma4Processor: UserInputProcessor {
    ...
    private func preprocessAudio(_ audio: UserInput.Audio) throws -> Gemma4PreparedAudio {
        ...
        guard let featureConfig = config.featureExtractor else {
            let tokenCount = min(
                ...
                max(
                    ...
                    Int(
                        Foundation.ceil(
                            ((Double(sampleCount) / Double(sampleRate)) * 1000.0)
                                / Double(config.audioMsPerToken ?? 40)
                        )
                    )
                )
            )
            let features = MLXArray(waveform).expandedDimensions(axis: 0).expandedDimensions(axis: -1)
            let mask = MLXArray(Array(repeating: false, count: sampleCount))
                .expandedDimensions(axis: 0)
            return Gemma4PreparedAudio(
                processed: LMInput.ProcessedAudio(
                    features: features,
                    mask: mask,
                    seqLengths: [sampleCount]
                ),
        ...
        let extracted = Gemma4AudioFeatureExtractor(config: featureConfig).extract(waveform: waveform)
        ...
        return Gemma4PreparedAudio(
            processed: LMInput.ProcessedAudio(
                ...
                mask: extracted.invalidMask,
                seqLengths: [extracted.features.dim(1)]
            ),
            sampleCount: extracted.sampleCount,
            sampleRate: featureConfig.samplingRate,
            tokenCount: tokenCount
    ...
    public func prepare(input: UserInput) async throws -> LMInput {
        ...
        if input.audio.count > 1 {
            throw Gemma4AudioProcessingError.multipleAudioInputs(input.audio.count)
        }

        var processedImage: LMInput.ProcessedImage?
        var processedAudio: LMInput.ProcessedAudio?
        var preparedAudio: Gemma4PreparedAudio?
        ...
        if let audio = input.audio.first {
            let audioPayload = try preprocessAudio(audio)
            preparedAudio = audioPayload
            processedAudio = audioPayload.processed
            if let featureExtractor = config.featureExtractor {
                let frameLength = Int(
                    round(Double(featureExtractor.samplingRate) * 20 / 1000.0)
                )
                print(
                    "[AUDIO] prompt prepared — samples=\(audioPayload.sampleCount), "
        ...
        if case .chat(let chatMessages) = input.prompt {
            ...
            if let preparedAudio {
                expandedTokens = expandAudioTokens(in: expandedTokens, audioSoftTokenCount: preparedAudio.tokenCount)
            }
            ...
            if let preparedAudio {
                prompt = expandAudioTokens(in: prompt, audioSoftTokenCount: preparedAudio.tokenCount)
            }
        ...
        if let preparedAudio {
            ...
            print(
                "[AUDIO] token expansion — "
                    ...
                    + "planned=\(preparedAudio.tokenCount), "
